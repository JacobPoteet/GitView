...
| `src/hooks/` | State that left `App.tsx`: `useInbox`, `useBatch`, `useUpdate`, `useTour`. Each owns its state and returns what the render needs; a feature that grows a handful of state goes here rather than into `App` |
...
| A command aimed at a repository goes to a shell at its prompt | A shell running `claude`, another CLI or a dev server reads typed text as its own input, so `gh pr merge` became a message to Claude. `pickShell` in `lib/sessions.ts` takes the tab on screen if it is free, then the first free shell, then a new one; busy means an OSC 133 block is still running, from `isBusy`, with no list of tool names. `emit` and every `pendingCommand` go through it. The Claude tab never qualifies. It stays asleep until clicked, and it stays asleep until clicked, and is not an agent panel: it types `claude` at a prompt and GitView adds no UI around it |
...
| Claude's state is read from what Claude prints, and nothing is written to `~/.claude` | Hooks are how Orca learns working, waiting and done, and they live in the user's own Claude settings. The OSC 0 title already says working (`◐`, `◑`) or idle (`✳`), and a question's `Esc to cancel` footer at the bottom of the screen separates waiting from done. `lib/agent.ts` holds the reading; `readAgent` in `TerminalPane.tsx` feeds it. The wiki's Terminal note has the titles as measured |
...
| A branch another worktree has checked out is never typed into a delete on its own | git refuses it, and in a prune one refusal fails the whole repository. `read_worktrees` in `fleet.rs` fills `BranchSummary.worktree` and keeps such a branch out of `merged_branches`; the branch menu types `git worktree remove` (or `prune` for a folder already gone) ahead of the delete, after closing GitView's own shells there, since Windows will not delete a folder a shell is standing in. Claude Code's worktrees sit under `.claude/worktrees/`, below where discovery looks, so this list is the only place GitView sees them |
...
| A font size is one of the six `--fs-` tokens, and the mono face is for what a prompt would accept | Fourteen literal sizes had crept into a 10 to 12.5 px band, and mono on pane titles and the status bar meant it said nothing. A path, a sha, a branch, a ref or a command is mono; a title, a count, a verb or a chord is not. A glyph used as an icon keeps a literal size. No label is uppercase. The wiki's `Interface/Theme.md` has the scale |
