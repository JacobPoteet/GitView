...
| A command aimed at a repository never lands in its Claude tab | That tab is a conversation, and `git fetch` typed there is a message. `shellOf` in `App.tsx` picks the shell that was on screen before Claude and brings it back to take the command; `emit` and every `pendingCommand` go through it. The Claude tab itself stays asleep until clicked, and is not an agent panel: it types `claude` at a prompt and GitView adds no UI around it |
