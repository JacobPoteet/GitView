...
Rust on Windows needs the MSVC linker, which a Rust install alone does not provide. Without Visual
Studio Build Tools and the C++ workload, `cargo build` fails at the link step on every crate with a
build script. CI on `windows-latest` has it.
